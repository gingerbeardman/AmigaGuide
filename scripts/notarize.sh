...
create_dmg() {
    ...
    rm -rf "$stage"
    print_success "DMG created: $DMG_PATH ($(du -h "$DMG_PATH" | cut -f1))"
...
main() {
    ...
    echo "  gh release create v${VERSION} \"$DMG_PATH\" --title \"$APP_NAME ${VERSION}\" --notes-file -"
}
