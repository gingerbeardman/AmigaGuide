...
static LONG PutHrefFrag(BPTR fh, const char *s)
{
  unsigned char ch;
  while ((ch = (unsigned char)*s++)) {
    if (ch == '&') {
      if (FPuts(fh, "&amp;") == -1) return -1;
    } else if (FPutC(fh, ch) == -1) {
      return -1;
    }
  }
  return 0;
...
static LONG PutAHrefOpen(BPTR fh, const char *url)
{
  if (FPuts(fh, "<a href=\"") == -1) return -1;
  if (PutHrefFrag(fh, url) == -1) return -1;
  return FPuts(fh, "\">");
}
      ...
      for(;;)
      {
        if('\"' == *buf)                  // is it a link?
        {
          ...
          if(Strnicmp(link,"link",4) == 0)
          {
			  ...
			  if(-1 == PutHrefFrag(fh,linkstr)) return(0);
  ...
  for(;;)
  {
	...
	if(param.toc)
	{
		if(enode->Count)  {          // main page does not have a TOC link
    		...
    		{
      			if(before) if(-1 == MyPuts(tfh,param.bar)) break;
      			if(-1 == PutAHrefOpen(tfh,enode->TOC)) break;
    		...
    		{
      			if(before) if(-1 == MyPuts(tfh,param.bar)) break;
      			if(-1 == PutAHrefOpen(tfh,TOC)) break;
      			if(-1 == MyPuts(tfh,param.toc)) break;
      			if(-1 == FPuts(tfh,"</a>")) break;
      			before = 1;
    		}
	...
	if(param.index)
	{
    	...
    	{
      		if(before) if(-1 == MyPuts(tfh,param.bar)) break;
      		if(-1 == PutAHrefOpen(tfh,enode->Index)) break;
    	...
    	{
      		if(before) if(-1 == MyPuts(tfh,param.bar)) break;
      		if(-1 == PutAHrefOpen(tfh,Index)) break;
      		if(-1 == MyPuts(tfh,param.index)) break;
      		if(-1 == FPuts(tfh,"</a>")) break;
      		before = 1;
    	}
	...
	if(param.help)
	{
    	...
    	{
      		if(before) if(-1 == MyPuts(tfh,param.bar)) break;
      		if(-1 == PutAHrefOpen(tfh,enode->Help)) break;
    	...
    	{
      		if(before) if(-1 == MyPuts(tfh,param.bar)) break;
      		if(-1 == PutAHrefOpen(tfh,Help)) break;
      		if(-1 == MyPuts(tfh,param.help)) break;
      		if(-1 == FPuts(tfh,"</a>")) break;
      		before = 1;
    	}
...
//      if(-1 == FPuts(tfh,"</a>")) break;
	...
	if(param.prev)
	{
    	...
    	{
      		if(before) if(-1 == MyPuts(tfh,param.bar)) break;
      		if(-1 == PutAHrefOpen(tfh,enode->Prev)) break;
    	...
    	else
    	{
      		...
      		{
        		if(before) if(-1 == MyPuts(tfh,param.bar)) break;
        		if(-1 == PutAHrefOpen(tfh,pred->Node)) break;
        		if(-1 == MyPuts(tfh,param.prev)) break;
        		if(-1 == FPuts(tfh,"</a>")) break;
        		before = 1;
      		}
	...
	if(param.next)
	{
    	...
    	{
      		if(before) if(-1 == MyPuts(tfh,param.bar)) break;
      		if(-1 == PutAHrefOpen(tfh,enode->Next)) break;
    	...
    	else
    	{
      		...
      		{
        		if(before) if(-1 == MyPuts(tfh,param.bar)) break;
        		if(-1 == PutAHrefOpen(tfh,next->Node)) break;
        		if(-1 == MyPuts(tfh,param.next)) break;
        		if(-1 == FPuts(tfh,"</a>")) break;
        		before = 1;
      		}
...
//if(!param.nohtml)
{
      ...
      if(-1 == FPuts(tfh,">\n")) goto ErrorFile;
		...
		if(param.singlefile)
		{
	        ...
	        if(-1 == PutHrefFrag(tfh,nodename)) goto ErrorFile;
			if(-1 == FPuts(tfh,"\" id=\"")) goto ErrorFile;
	        if(-1 == PutHrefFrag(tfh,nodename)) goto ErrorFile;
			if(-1 == FPuts(tfh,"\">")) goto ErrorFile;
		      if(-1 == MyPuts(tfh,title)) goto ErrorFile;
...
Flush:
    ...
    if(htmltop) { FreeMem(htmltop,hhs); htmltop = NULL; }
    if(htmlbot) { FreeMem(htmlbot,hfs); htmlbot = NULL; }
